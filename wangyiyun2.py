...
class Worker(QThread):
    ...
    def __init__(self, files, save_dir, ncmdump_path, keep_cover, target_fmt):
        ...
        self.target_fmt = target_fmt  # 用户选择的格式 (alac, flac, mp3...)
    ...
    def process_ncm_decrypt(self, file_path, filename):
        ...
        # 寻找解密产物 (ncmdump 通常输出 mp3 或 flac)
    ...
    def convert_ffmpeg(self, inp, out, sample_rate):
        ...
        # === 封面流处理 (修复点) ===
        # OGG 和 WAV 都不支持这种流式封面嵌入，强行嵌入会报错，所以必须跳过
        ...
        # 3. FLAC (无损)
            ...
            cmd.extend(['-c:a', 'flac'])
            # 智能位深：>48k 保持原样(或24bit)，<=48k 强制16bit以节省空间
...
class UniversalCommander(QMainWindow):
    def __init__(self):
        ...
        if os.path.exists(self.ncmdump_path):
            ...
            lbl_status.setText("⚠️ 未检测到 ncmdump.exe (NCM 解密将跳过)")
